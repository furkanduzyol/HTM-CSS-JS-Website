...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // --- Topic -> Other seçilince input açılacak ---
    if (topic && otherInput) {
    ...
    // --- Selectlerin placeholder rengi ---
    const selects = document.querySelectorAll('.form-control-select');

    selects.forEach(select => {
        select.addEventListener('change', function () {
            if (this.value === "") {
                this.style.color = "#777"; // placeholder gibi
            } else {
                this.style.color = "#000"; // normal seçim
            }
        });
    });
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (contactForm) {  // sadece form varsa çalışsın
...
,
     program_title: "PROGRAM",
        ...
        day2_list: `
            ...
            <p><strong>12:00 - 14:00 Lunch</strong></p>
            <p><strong>14:00 - 17:30 Oral Presentations</strong></p>
...
oral_title: `<h2>Oral Presentation Instruction:</h2>`,
...
poster_title: `<h2>Poster Presentation Instruction:</h2>`,
...
oral_title: `<h2>Instruções para Apresentação Oral:</h2>`,
...
poster_title: `<h2>Instruções para Apresentação de Poster:</h2>`,
...
function applyLang(lang) {
    document.querySelectorAll("[data-i18n]").forEach(el => {
        ...
        if (translations[lang][key]) {
            el.innerHTML = translations[lang][key]; // innerHTML kullanıyoruz
        }
    });
...
// Sayfa açıldığında kaydedilen dili uygula
const savedLang = localStorage.getItem("site-lang") || "en";
applyLang(savedLang);

// Dropdown’u seçilen dile ayarla
document.getElementById("lang").value = savedLang;

// Dil değiştiğinde applyLang fonksiyonunu çağır
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Correct event date (year, monthIndex (0–11), day, hour, minutes, seconds)
...
//Ana sayfadaki fotoğrafların kayması için js kodu
let index = 0;
const slides = document.querySelectorAll(".slide");
const container = document.querySelector(".slider-container");

const total = slides.length; // 3 oldu (2 gerçek + 1 kopya)
...
function nextSlide() {
    ...
    // SON SLIDE (kopya) GELİNCE anında başa dön
    if (index === total - 1) {
        setTimeout(() => {
            container.style.transition = "none"; // kayma animasyonunu kapat
            container.style.transform = "translateX(0)"; // en başa ışınla
            index = 0; // tekrar başla
        }, 800); // animasyon tamamlanınca
    }
}
...
// Mouseover ile durdur, mouseout ile başlat
